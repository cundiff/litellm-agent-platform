...
It is a thin bridge: each session spawns the existing `../server/server.mjs` subprocess
...
# Watch replies — SSE stream; pipe through jq to see agent.message events
curl -s -N $BASE/v1/sessions/$ID/events/stream \
  | grep '^data:' | sed 's/^data: //' | jq '{type:.type, content:.content}'

# Or poll history after the turn is done (session.status_idle):
...
Tests live at `tests/src/lite-harness-sdk/managed-agents/` (repo root).

**Offline (deterministic, no network/key/SDK):** spawns a fake harness.

```bash
T=tests/src/lite-harness-sdk/managed-agents
